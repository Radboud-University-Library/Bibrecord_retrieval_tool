...
- Uses a fixed worker pool (`4`) with global API pacing at `2 requests/second`.
...
- Step-by-step export flow:
  - Step 2: Generate Excel with a status stepper and progress text per phase.
...
- Compact error reporting via expandable "Detailed errors" and grouped "Why records were not fetched" sections.
...
5. After all records are retrieved and saved successfully:
   - Click "Step 2: Generate Excel". The app will export XML, optionally export JSON holdings (if present), and merge them. A status panel describes each step.
